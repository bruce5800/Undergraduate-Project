...
\textbf{scheduling-time cost}: NSGA-II re-runs a
population-based search at dispatch time---$5.4\times$ our
scheduler's total wall-clock per simulation
($\approx$70~ms of search per dispatched task, measured),
whereas our policy amortizes learning into a one-off pretrain
and decides by a sub-millisecond forward pass. In
latency-sensitive serving, dispatch-time search directly
inflates the very TTFT the scheduler is trying to protect;
we therefore read NSGA-II as the strongest \emph{offline}
competitor, and its strength at saturation
(\S\ref{sec:load}) sharpens rather than undermines the
regime characterization (C4).
