...
\textbf{Baselines (10 schedulers).} We compare against a broad
set:
\begin{itemize}
...
All ten schedulers are implemented natively in Python against
the simulator's uniform \texttt{scheduler.schedule()}
interface; no external scheduler library is reused. HEFT
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}l@{\hspace{6pt}}p{0.64\columnwidth}@{}}
    ...
    \multicolumn{2}{@{}p{0.97\columnwidth}@{}}{\emph{Training
      budget (all three learned schedulers):} 20 pretraining
      episodes on the target configuration
      ($\approx$10--15~s wall-clock), initialized by 30
      uniform-random warm-up dispatches; online updates
...
We evaluate all ten schedulers on the canonical configuration:
...
all nine baselines} in the (\slo, energy efficiency) plane
...
1.3\% gap). Makespan is not among our optimization objectives
(\S\ref{sec:sched-problem}), and the gap does not affect the
Pareto relation on the (\slo, \etok) plane, where our scheduler
...
comparing our RL scheduler against each of the five strongest
baselines.

\begin{table}[t]
  \centering
  \caption{$p$-values vs.\ five strongest baselines (two-sided
  ...
  \begin{tabular}{lccccc}
    \toprule
    Metric & PSO & SQ & A3C-R2N2 & GNN & CS-UCB \\
    \midrule
    \slo{} attain.   & \textbf{0.045}$^*$ & \textbf{0.046}$^*$
                     & \textbf{0.006}$^{**}$ & \textbf{0.006}$^{**}$
                     & \textbf{0.002}$^{**}$ \\
    Total energy     & 0.36 & 0.07 & 0.09 & 0.10 & 0.21 \\
...
against all five baselines ($p<0.05$, with $p<0.01$ for both
DRL baselines and the bandit). Energy efficiency is
significantly better
against three of five (PSO, $p=0.11$, and CS-UCB, $p=0.066$,
are directionally consistent but
do not reach $p<0.05$). Total energy shows a consistent
...
PerLLM~\cite{yang2024perllm}. Yet it lands squarely in the
baseline cluster (\slo{} 0.193, \etok{} 2.84): significantly
below our scheduler on \slo{} ($p{=}0.002$) and directionally
worse on energy ($p{=}0.066$). The gap has a structural
cause: stationary per-arm mean rewards cannot track
\emph{state-dependent} costs---whether a model is resident or
a batch has free slots depends on the cluster's evolving
state, which a context-free bandit averages away while a deep
policy conditions on it. Constraint filtering alone,
therefore, does not substitute for state-conditioned
...
\begin{table}[t]
  ...
  \caption{\slo{} and \etok{} (J/tok) vs.\ edge count.
    $\bigstar$ = column-best across all 10 schedulers.}
  \label{tab:topology}
  \scriptsize
  ...
  \begin{tabular}{lrrrrrr}
    \toprule
    Config (1 cloud +)
      & PSO & SQ & A3C-R2N2 & GNN & CS-UCB & \textbf{RL} \\
    \midrule
    3 edges --- \slo
      & 0.273 & 0.277 & 0.246 & 0.241 & 0.243
      & \textbf{0.314}$^\bigstar$ \\
    3 edges --- \etok
      & 2.19 & 2.32 & 2.28 & 2.24 & 2.24
      & \textbf{1.99}$^\bigstar$ \\
    5 edges --- \slo
      & 0.235 & 0.234 & 0.214 & 0.210 & 0.193
      & \textbf{0.302}$^\bigstar$ \\
    5 edges --- \etok
      & 2.81 & 2.96 & 2.93 & 2.93 & 2.84
      & \textbf{2.61}$^\bigstar$ \\
    7 edges --- \slo
      & 0.241 & 0.218 & 0.193 & 0.250 & 0.152
      & \textbf{0.282}$^\bigstar$ \\
    7 edges --- \etok
      & 3.25 & 3.34 & 3.35 & 3.35 & 3.35
      & \textbf{3.00}$^\bigstar$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Tables~\ref{tab:slo-vs-lambda} and~\ref{tab:e-vs-lambda}
report \slo{} attainment and energy per token, respectively.
...
\begin{table}[t]
  ...
  \begin{tabular}{rrrrrrl}
    \toprule
    $\lambda$ & PSO & SQ & A3C-R2N2 & GNN & CS-UCB & \textbf{RL} \\
    \midrule
    0.5 & 0.596 & 0.339 & 0.359 & 0.500 & 0.462
        & \textbf{0.596}$^\dagger$ \\
    1.0 & 0.297 & 0.265 & 0.227 & 0.237 & 0.214
        & \textbf{0.447}$^\bigstar$ \\
    2.0 & 0.235 & 0.234 & 0.214 & 0.210 & 0.193
        & \textbf{0.302}$^\bigstar$ \\
    4.0 & \textbf{0.253} & 0.205 & 0.191 & 0.207 & 0.215
        & 0.214 \\
    8.0 & \textbf{0.216} & 0.167 & 0.156 & 0.193 & 0.187
        & 0.199 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{rrrrrrr}
    ...
    $\lambda$ & PSO & SQ & A3C-R2N2 & GNN & CS-UCB & \textbf{RL} \\
    \midrule
    0.5 & 3.00 & 3.49 & 3.38 & 3.13 & 3.19
        & \textbf{2.96}$^\bigstar$ \\
    1.0 & 2.77 & 3.00 & 3.07 & 2.95 & 2.93
        & \textbf{2.64}$^\bigstar$ \\
    2.0 & 2.81 & 2.96 & 2.93 & 2.93 & 2.84
        & \textbf{2.61}$^\bigstar$ \\
    4.0 & 2.85 & 2.97 & 2.94 & 2.92 & 2.86
        & \textbf{2.58}$^\bigstar$ \\
    8.0 & 2.94 & 2.89 & 2.98 & 2.91 & 2.99
        & \textbf{2.52}$^\bigstar$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
better than RL's incremental policy---and even CS-UCB reaches
RL's \slo{} level at $\lambda{=}4$ (0.215 vs.\ 0.214),
confirming that saturation erases policy differences on the
\slo{} axis. \textbf{However, RL's
...
that our scheduler Pareto-dominates nine baselines---yet its
...
\S\ref{sec:main-results} (RL strictly dominates 9 baselines)
...
problem into a scalar weighted sum, raising the question of
whether our conclusions are an artifact of one particular
weight vector. We therefore retrain the scheduler from scratch
under four alternative weight configurations spanning the
design space---\emph{uniform} (no prior: all seven weights
$1/7$), \emph{time-heavy} (the time term receives over half
the total mass), \emph{AIGC-heavy}
(warm{+}batch{+}affinity raised to 0.70 total), and
\emph{base-heavy} (AIGC terms nearly removed, 0.15
total)---each with $N{=}30$ runs at the canonical operating
point. Table~\ref{tab:weight-sens} reports the results.
...
reinforces the interpretation that the gain arises from the
training procedure and simulator physics rather than from a
finely tuned reward vector. Conversely, it also means our
scalarization is benign: within a wide region of weight
space, the learned policy lands on the same Pareto point, so
the conclusions drawn from the canonical configuration
generalize across reasonable weight choices.
